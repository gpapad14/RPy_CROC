...
/* GPIO 4 use as RD */
#define GP4_HI *gpset = (1 << 4)  // GPIO 4 output hi
#define GP4_LO *gpclr = (1 << 4)  // GPIO 4 output lo
/* GPIO 5 use as RU */
#define GP5_HI *gpset = (1 << 5)  // GPIO 5 output hi
#define GP5_LO *gpclr = (1 << 5)  // GPIO 5 output lo
...
int main()
  ...
  printf("ASPIC test starting\n");
  ASPICtest();
  printf("ASPIC test ended\n");
  return(0);
  }
...
void ASPICtest()
  ...
  INP_GPIO(4);   // must do this first to zero control bits
  OUT_GPIO(4);   // set as output
  INP_GPIO(5);   // must do this first to zero control bits
  OUT_GPIO(5);   // set as output
  ...
  for(n = 0 ;  n < 1000000  ; ++n)
    ...
    timend += 5;    // wait time before RD
    TIMERLOOP;

    GP4_HI;          // output GPIO 4 hi
    timend += 2;    // 2us  RD
    TIMERLOOP;
    GP4_LO;         // output GPIO 4 lo

    timend += 5;    // wait time before RU
    TIMERLOOP;

    GP5_HI;          // output GPIO 5 hi
    timend += 2;    // 2us  RD
    TIMERLOOP;
    GP5_LO;         // output GPIO 5 lo
